...
sim_table %<>% filter(handle_mislabels != "local_search_correct") %>% 
    filter(!(handle_mislabels == "majority_cycles_swap_entropy_correct_and_remove" & n_mislabels > 150))
```
...
elist <- mislabel_handlers$majority_cycles_swap_entropy_correct_and_remove(elist)
