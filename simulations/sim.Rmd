...
mislabel_handlers <- list(
    ...
    #     ## For each subject, if there is a consensus among the samples
    #     ## remove minority samples for that subject
    #     ## If there isn't a consensus among samples for that subject
    #     ## remove every sample for that subject
    #     sample_mislabel_table <- elist$targets %>%
    #         select(Sample_ID, Subject_ID, Mislabeled, E_Sample_ID, E_Subject_ID) %>% 
    #         group_by(Subject_ID) %>%
    #         mutate(
    #             n_votes = n(),
    #             Majority = ifelse(any(table(E_Subject_ID) > n_votes/2), mode(E_Subject_ID), NA)
    #         )
    #         mutate(
    #             Consensus = Subject_Pct_Mislabeled < 0.5,
    #             Minority = 
    #             Remove = (Consensus & Minority) | !Consensus
    #         )
    #     return(elist[, !sample_mislabel_table$Remove])
    # }
