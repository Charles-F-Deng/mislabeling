...
# Helpers for correction algorithm
...
validate_sample_genotype_data <- function(sample_genotype_data) {
    assert_that(
        ...
        msg = "Invalid input: param 'sample_genotype_data' must be a data frame"
    )
    ...
    assert_that(
        ...
        msg = glue("Invalid input: param 'sample_genotype_data' is missing required column(s) {paste(missing_columns, collapse=\", \")}")
    )
       
    duplicated_samples <- sample_genotype_data$Sample_ID[duplicated(sample_genotype_data$Sample_ID)]
    assert_that(
        length(duplicated_samples) == 0,
        msg = glue("Invalid input: param 'sample_genotype_data' has non-unique Sample_ID(s) {paste(duplicated_samples, collapse=\", \")}")
    )
}

find_directed_cycles <- function(graph, cutoff=1) {
    assert_that(is_directed(graph), 
                msg="Graph is not directed")
    cycles <- list()
...
max_component_subject_ids <- function(elist) {
    ...
    subject_id_counts <- elist$targets[, c("Sample_ID", "Subject_ID", "Genotype_Group_ID")] %>% 
        mutate(
...
generate_graph = function(sample_genotype_data, graph_type=names(graph_type_mapping), plot=FALSE, filter=FALSE) {
...
## For simulation use only - vertex colors represent true mislabel status
generate_graph_answer_key = function(elist, graph_type=names(graph_type_mapping), oracle=FALSE, plot=FALSE, filter=FALSE) {
    graph_type_mapping <- list(
        label = "Subject_ID",
        genotype = "Genotype_Group_ID",
        combined = NA_character_
...
## For simulation use only - vertex colors represent true mislabel status
generate_corrections_graph_answer_key <- function(elist, oracle=FALSE, plot=FALSE) {
...
get_majority_majority_pairs <- function(connected_component) {
    ...
    putative_subjects <- inner_join(votes_by_subject, votes_by_genotype, by=c("Subject_ID", "Genotype_Group_ID"))
    return(putative_subjects)
}

# 1. For each Genotype, determine the unique Majority_Subject_ID
# 2. For each Subject_ID, determine unique Majority_Genotype_Group_ID
...
get_majority_majority_relabels_graph <- function(elist) {
    ...
    assert_that(!any(duplicated(putative_subjects$Subject_ID)) &
                !any(duplicated(putative_subjects$Genotype_Group_ID)),
                msg="Non-unique Subject_ID/Genotype_Group_ID pair")
    putative_subjects %<>% column_to_rownames("Genotype_Group_ID")
    
    potential_mislabel_univ <- elist$targets %>% 
        select(Sample_ID, Subject_ID, Genotype_Group_ID) %>% 
        mutate(
            Putative_Subject_ID = sapply(Genotype_Group_ID, \(x) putative_subjects[x, ][[1]]),
    ...
    for (sample_id in potential_mislabel_univ) {
        Genotype_Group_ID = elist$targets[sample_id, ]$Genotype_Group_ID[[1]]
        inferred_subject_id = putative_subjects[Genotype_Group_ID, ][[1]]
...
calc_entropy = function(labels) {
    labels %>% group_by(Genotype_Group_ID) %>%
