...
swap_elist_samples <- function(elist, swaps) {
    assert_that(all(unique(as.vector(swaps)) %in% elist$targets$Sample_ID), 
                ## TODO output the Sample_ID that doesn't belong
                msg = "Cannot swap samples that aren't part of Sample_ID set")
...
get_potential_relabels_list <- function(elist) {
    ...
    inferred_mislabel_univ <- elist$targets %>% filter(Non_Majority | is.na(Non_Majority)) %>% pull(Sample_ID)
...
mislabel_handlers <- list(
    ...
    iterative_majority = function(elist) {
        ## Solve smaller cycles first
        max_cycle_size <- 2
        while (TRUE) {
            print(max_cycle_size)
            potential_relabels_list <- get_potential_relabels_list(elist)
            
            ## Don't need this if I can find a way to set column to a vector of zero elements
            ## w/o deleting that column
            ...
            cycle_tree <- data.frame(cycle_head = names(potential_relabels_list)) %>% 
                mutate(
                    cycle_path = cycle_head,
                    cycle_tail = cycle_head,
                    cycle_size = 1
                )
            for (curr_cycle_size in seq.int(2, max_cycle_size + 1)) {
                ...
                ) %>% unnest(cycle_tail) %>% mutate(
                    cycle_size = curr_cycle_size,
                    cycle_path = paste(cycle_path, cycle_tail, sep = "|")
                )
                cycle_tree <- rbind(cycle_tree, cycle_leaf_nodes)
            }
            cycle_tree %<>% 
                ## Filter in complete cycles only
                filter(cycle_size > 1 & cycle_head == cycle_tail) %>% 
                ## Filter out ambiguous cases where we complete a cycle in 2 different ways
                distinct(cycle_head, cycle_tail, cycle_size, .keep_all = TRUE) %>% arrange(cycle_size)
            
            ## Stopping condition
            if (nrow(cycle_tree) == 0 & max_cycle_size >= length(potential_relabels_list)) {
                break
            } else if (nrow(cycle_tree) == 0) {
                ## If no cycles have been detected, continue
                max_cycle_size <- max_cycle_size + 1
                next
            }
            
            ## Correct any detected cycles
            corrected_labels <- c()
            for (i in 1:nrow(cycle_tree)) {
                cycle_path <- unlist(strsplit(cycle_tree$cycle_path[[i]], "\\|"))
                cycle_path <- cycle_path[-c(length(cycle_path))]
                ## Do nothing if the path includes an already corrected sample
                if (length(intersect(corrected_labels, cycle_path)) != 0) {
                    next
                }
                ## Use cycle path to create swap matrix and fix labels
                swaps <- cbind(
                    cycle_path[[1]],
                    rev(cycle_path[-c(1)])
                )
                elist <- swap_elist_samples(elist, swaps)
                corrected_labels <- c(corrected_labels, cycle_path)
            }
        }
        return(elist)
    }
    ...
    label_outcomes <- elist$origs$targets_orig %>%
        ...
        mutate(
            Outcome = case_when(
                ...
                !Mislabeled_final ~ "correctly relabeled",
                Mislabeled_final ~ "incorrectly relabeled",
                .default = NA
            )
...
grid_list <- list(
    ...
    tibble(n_mislabels = seq(from = 0, to = 40, by = 10)),
...
num_simulations <- sum(need_to_run)
...
run_sim_with_progress <- function(...) {
    result <- try(run_sim(...), silent = FALSE)
    pb$tick()
    return(result)
}

if (any(need_to_run)) {
    tsmsg("Running ", num_simulations, " simulations")
    sim_result_table %<>% 
        filter(need_to_run) %>% 
        mutate(
            results_list = {
                map_args <- list(f = run_sim_with_progress)
                map_args[sim_param_names] <- .[sim_param_names]
                do.call(future_Map, map_args)},
            top_table = lapply(results_list, \(x) try(x$tt, silent = TRUE)),
            label_outcomes = lapply(results_list, \(x) try(x$label_outcomes, silent = TRUE))
        ) # %>% select(-results_list)
    need_to_run <- sapply(sim_result_table$top_table, is, "try-error")
...
sim_result_label_outcomes_table <- sim_result_table %>% 
    ...
    mutate(
        Mislabeled_orig = ifelse( Mislabeled_orig, "mislabels", "correct labels" )
    ) %>% 
    pivot_wider(
...
```{r}
## Interesting edge case where there is no majority and 2/5 samples have
## a mislabel from the same subject_id (2-1-1-1 (correct))
elist_params <- list(
  n_subjects_per_group = 30,
  n_samples_per_subject = 5,
  n_features = 5,
  ...
  seed = 1988)
elist <- do.call(sim_mislabeled_data, elist_params)
elist <- mislabel_handlers$iterative_majority(elist)
