...
plot_graph <- function(graph) {
    ...
    my_plot <- plot(graph, vertex.size=4, vertex.label.cex=0.6, edge.arrow.size=.8, edge.width=5, 
...
local_search_correct <- function(sample_genotype_data, objective, swap_cats_graph, n_iter=1, filter_concordant_vertices=TRUE) {
    ...
    find_neighbors <- function(combined_graph, swap_cats_graph) {
        ...
        filtered_unique_pairs <- inner_join(unique_pairs, swap_cats_edges, by=c("Sample_A", "Sample_B"))
        return(unique_pairs)
    }
    ...
    for (curr_iter in 1:n_iter) {
        ...
        neighbors <- find_neighbors(combined_graph, swap_cats_graph) %>% 
...
correct_main <- function(sample_genotype_data, swap_cats=NULL) {
    ...
    components <- clusters(combined_graph)$membership
    ...
    cluster_info <- clusters(corrections_graph)
...
genswaps_random_with_replacement = function(elist, n_mislabels, swap_cats) {
    ...
    swap_cats_graph <- swap_cats_to_graph(swap_cats)
    components <- clusters(swap_cats_graph)
