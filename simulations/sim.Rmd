...
validate_sample_genotype_data <- function(sample_genotype_data, init_required=FALSE) {
    assert_that(
        ...
        msg = "Invalid: 'sample_genotype_data' must be a data.frame object"
    )
...
validate_putative_subjects <- function(putative_subjects) {
    assert_that(
        ...
        msg = "Invalid: 'putative_subjects' must be a data.frame object"
    )
...
validate_relabels <- function(sample_genotype_data, relabels) {
    ...
    assert_that(
        ...
        msg = "Invalid: 'relabels' must be a data.frame object"
    )
...
validate_swap_cats <- function(swap_cats, sample_genotype_data=NULL) {
    assert_that(
        is.data.frame(swap_cats),
        msg = "Invalid: 'swap_cats' must be a data.frame object"
    )
    
    assert_that(
        colnames(swap_cats)[1] == "Sample_ID",
        msg = "Invalid: 'swap_cats' first column must have name Sample_ID"
    )
    
    duplicated_samples <- swap_cats$Sample_ID[duplicated(swap_cats$Sample_ID)]
    assert_that(
        length(duplicated_samples) == 0,
        msg = glue("Invalid: 'swap_cats' has non-unique Sample_ID(s) {paste(duplicated_samples, collapse=\", \")}")
    )
    ...
    if (ncol(swap_cats) > 1) {
        ...
        assert_that(
            ...
            msg = glue("Invalid: 'swap_cats' columns 2:n must be of type factor, check column(s) {paste(non_factor_cols, collapse=\", \")}")
        )
    }
    
    if (!is.null(sample_genotype_data)) {
        missing_samples <- setdiff(sample_genotype_data$Sample_ID, swap_cats$Sample_ID)
        assert_that(
            length(missing_samples) == 0,
            msg = glue("Invalid: 'swap_cats' is missing Sample_ID(s) that exist in 'sample_genotype_data', check Sample_ID(s) {paste(missing_samples, collapse=\", \")}")
        )
    }
}

get_unsolved_components <- function(sample_genotype_data) {
    validate_sample_genotype_data(sample_genotype_data)
    solved_components <- sample_genotype_data %>% 
        ...
        pull(Component_ID)
    return(setdiff(unique(sample_genotype_data$Component_ID), solved_components))
}
...
generate_graph = function(sample_genotype_data, graph_type=names(graph_type_mapping), filter_components=FALSE, plot=FALSE) {
    ...
    if (graph_type == "combined") {
        ...
        if (filter_components) {
            V(graph)$temp_component_id <- components(graph)$membership
...
generate_corrections_graph <- function(sample_genotype_data, plot=FALSE) {
    validate_sample_genotype_data(sample_genotype_data, init_required=TRUE)
...
swap_cats_to_matrix <- function(swap_cats) {
    # validate_swap_cats(swap_cats)
    n <- nrow(swap_cats)
    if (ncol(swap_cats) < 2) {
    ...
    return(swap_cats_matrix)
} 


swap_cats_to_graph <- function(swap_cats) {
    swap_cats_matrix <- swap_cats_to_matrix(swap_cats)
...
generate_relabels_graph_from_putative_subjects <- function(sample_genotype_data, putative_subjects, swap_cats_graph) {
    validate_sample_genotype_data(sample_genotype_data)
    ...
    relabels_graph <- graph.intersection(relabels_graph, swap_cats_graph)
...
relabel_cycles <- function(relabels_graph) {
    ...
    all_cycles <- list()
    cutoff <- 1
    while (cutoff < max(table(components(relabels_graph)$membership))) {
...
min_swaps_comprehensive_search <- function(connected_component) {
    ...
    putative_subjects <- perm_genotypes[1, ] %>% 
        select(-n_min_mislabels, -n_implied_mislabels) %>% 
        t() %>% 
        as.data.frame() %>% 
        rownames_to_column()
    colnames(putative_subjects) <- c("Genotype_Group_ID", "Subject_ID")
    
    relabels_graph <- generate_relabels_graph_from_putative_subjects(connected_component, putative_subjects, swap_cats_graph)
    relabels <- relabel_cycles(relabels_graph)
    return(relabels)
    
    ## Old stuff that is to be replaced by the relabels_graph/detect_cycles framework
    # connected_component %<>%
    #     mutate(
...
local_search_correct <- function(sample_genotype_data, objective, swap_cats_graph, n_iter=1, filter_concordant_vertices=TRUE) {
    ...
    calc_swapped_objective <- function(relabel_data, swap_from, swap_to) {
...
correct_majority_cycles <- function(sample_genotype_data, swap_cats_graph) {
    putative_subjects <- get_majority_majority_pairs(sample_genotype_data)
    relabels_graph <- generate_relabels_graph_from_putative_subjects(sample_genotype_data, putative_subjects, swap_cats_graph)
...
correct_comprehensive_search <- function(sample_genotype_data, swap_cats_graph, max_subjects=9) {
    relabels <- data.frame(relabel_from=character(0), relabel_to=character(0))
    components <- unique(sample_genotype_data$Component_ID)
    for (component_id in components) {
        connected_component <- sample_genotype_data %>% filter(Component_ID == component_id)
...
correct_main <- function(sample_genotype_data, swap_cats=NULL) {
    validate_sample_genotype_data(sample_genotype_data)
    if (!is.null(swap_cats)) {
        validate_swap_cats(swap_cats, sample_genotype_data)
    } else {
        swap_cats <- sample_genotype_data %>% select(Sample_ID)
    }
    swap_cats_graph <- swap_cats_to_graph(swap_cats)
    
    ## Assign a Component_ID for each connected component in the graph
    combined_graph <- generate_graph(sample_genotype_data, graph_type="combined")
    components <- components(combined_graph)$membership
    component_sizes <- table(components)
    sorted_components <- order(component_sizes, decreasing=TRUE)
    ordered_components <- rep(NA_integer_, length(components))
    for (i in seq_along(sorted_components)) {
        formatted_component <- sprintf("%0*d", digits(length(component_sizes)), i)
        ordered_components[components == sorted_components[i]] <- glue("Component", formatted_component)
    }
    sample_genotype_data %<>% mutate(
        Component_ID = ordered_components,
        Init_Sample_ID = Sample_ID,
        Init_Subject_ID = Subject_ID,
        Init_Component_ID = ordered_components
    )
    orig_sample_genotype_data <- sample_genotype_data
    unsolved_components <- sort(get_unsolved_components(orig_sample_genotype_data))
    
    while (TRUE) {
        sample_genotype_data <- correct_comprehensive_search(sample_genotype_data, swap_cats_graph)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
        sample_genotype_data <- correct_majority_cycles(sample_genotype_data, swap_cats_graph)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
        sample_genotype_data <- correct_comprehensive_search(sample_genotype_data, swap_cats_graph)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
        sample_genotype_data <- correct_local_search_entropy(sample_genotype_data, swap_cats_graph, n_iter=1)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
    }
    ...
    ##########          Summary statistics          ##########
    ##########################################################
    sample_summary <- sample_genotype_data %>%
    ...
    corrections_graph <- generate_corrections_graph(sample_genotype_data)
    ...
    for (component in unsolved_components) {
        ...
        generate_graph(orig_sample_genotype_data %>% filter(Init_Component_ID == component), graph_type="combined", plot=TRUE)
        title(main=glue(component, ": Before corrections"), cex.main=2)
        generate_corrections_graph(sample_genotype_data %>% filter(Init_Component_ID == component), plot=TRUE)
        title(main=glue(component, ": Applied corrections"), cex.main=2)
        generate_graph(sample_genotype_data %>% filter(Init_Component_ID == component), graph_type="combined", plot=TRUE)
...
handle_mislabels_correct_majority_swaps <- function(elist) {
    ...
    V(unambiguous_relabels_graph)$cycle_id <- components(unambiguous_relabels_graph)$membership
...
handle_mislabels_comprehensive_search <- function(elist, max_subjects=9) {
    ...
    connected_components <- elist$targets[, c("Sample_ID", "Subject_ID", "Genotype_Group_ID")] %>% 
        mutate(
            Component_ID = str_c("Component_", components(combined_graph)$membership)
        )
...
sim_mislabeled_data <- function (
    ...
    if (n_swap_cats > 0) {
        swap_cat_names <- generate_ids("SwapCat", n_swap_cats)
        for (sci in swap_cat_names) {
            groups <- paste0(sci, "_Group", c(1,2))
            swap_cats[[sci]] <- as.factor(sample(groups, nrow(sample_meta_table), replace=TRUE))
        }
    }
    ...
    elist <- handle_swaps_function(elist, n_mislabels, swap_cats)
    
    # Remove genotype information from certain rownames(elist$targets)
    tsmsg(glue("Deleting genotype information from {percent(fraction_ghost)} of samples"))
    ghost_samples <- sample(rownames(elist$targets), round(fraction_ghost * n_samples), replace=FALSE)
    elist$targets[ghost_samples, "Genotype_Group_ID"] <- NA_character_
...
connected_component_distribution <- function(...) {
    ...
    connected_components <- elist$targets[, c("Sample_ID", "Subject_ID", "Genotype_Group_ID")]
    connected_components$Component_ID <- components(elist_graph)$membership
...
elist_params <- list(
  ...
  swap_model = "random_with_replacement",
  seed = 1988)
fraction_anchor_sample <- 0.05
elist <- do.call(sim_mislabeled_data, elist_params)
sample_genotype_data <- elist$targets %>%
    select(Sample_ID, Subject_ID, Genotype_Group_ID)
anchor_samples <- sample(elist$targets %>% 
    filter(!Mislabeled) %>% 
    pull(Sample_ID), 2*fraction_anchor_sample*(n_samples_per_group - n_mislabels))
rownames(sample_genotype_data) <- NULL
swap_cats <- elist$swap_cats

merged_df <- left_join(elist$targets[, c("Sample_ID", "E_Sample_ID")], 
          my_mislabel_solver@.solve_state$relabel_data[, c("Init_Sample_ID", "Sample_ID")], 
          by=c("Sample_ID"="Init_Sample_ID")) %>% 
    mutate(
        Init_Mislabeled = Sample_ID != E_Sample_ID,
        Final_Mislabeled = Sample_ID.y != E_Sample_ID
    )
...
# sample_genotype_data <- elist$targets %>% 
#     select(Sample_ID, Subject_ID, Genotype_Group_ID, E_Sample_ID) %>% 
#     mutate(
#         Init_Sample_ID = Sample_ID,
#         Sample_ID = E_Sample_ID
#     )
...
combined_graph <- generate_graph(sample_genotype_data, graph_type="combined")
sample_genotype_data$Component_ID <- components(combined_graph)$membership[sample_genotype_data$Sample_ID]
