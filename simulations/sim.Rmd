...
graph_type_mapping <- list(
        label = "Subject_ID",
        genotype = "E_Subject_ID",
        combined = NA_character_
    )
generate_graph = function(elist, graph_type=names(graph_type_mapping)) {
    ...
    if (graph_type == "combined") {
        ...
        return(igraph::union(genotype_graph, labels_graph))
    }
    ...
    edges <- elist$targets %>%
        ...
        distinct() %>%
        as.data.frame()
    ## Vertices has to be two columns
    vertices <- elist$targets[, c("Sample_ID", "Subject_ID")]
    graph <- graph_from_data_frame(edges, directed=FALSE, vertices=vertices)
    return(graph)
}

detect_cycles <- function(elist) {
    mislabeled_elist <- elist$targets %>% filter(Mislabeled)
    cycle_list <- vector("list", nrow(mislabeled_elist))
    i <- 1
    while (nrow(mislabeled_elist) > 0) {
        ## Start with first mislabeled Sample_ID
        curr_sample_id <- mislabeled_elist[1, ]$Sample_ID
        cycle <- c(curr_sample_id)
        while (TRUE) {
            next_sample_id <- mislabeled_elist[curr_sample_id, ]$E_Sample_ID
            ## The stop condition is if we've returned to the head of the current cycle
            if (next_sample_id == cycle[[1]]) {
                break
            } else {
                cycle <- c(cycle, next_sample_id)
                curr_sample_id <- next_sample_id
            }
        }
        cycle_list[[i]] <- cycle
        ## Filter out all the samples that were detected as part of this cycle
        mislabeled_elist <- mislabeled_elist %>% filter(!(Sample_ID %in% cycle))
        i <- i + 1
    }
    ## Filter out unused indices
    cycle_list <- cycle_list[!sapply(cycle_list, is.null)]
    return(cycle_list)
}

generate_cycles_graph <- function(elist) {
...
generate_corrections_graph <- function(elist, oracle=FALSE, plot=FALSE) {
    ...
    graph <- graph_from_data_frame(applied_relabels, directed=TRUE, vertices=applied_relabels) %>% 
        set_edge_attr("edge_colors", value="grey")
    
    if (oracle) {
        oracle_relabels <- elist$origs$targets_orig[, c("Sample_ID", "E_Sample_ID")] %>% 
            filter(Sample_ID != E_Sample_ID)
        oracle_graph <- graph_from_data_frame(oracle_relabels, directed=TRUE, vertices=oracle_relabels) %>% 
            set_edge_attr("is_in_oracle", value=TRUE)
        
        applied_graph <- graph %>% 
            set_edge_attr("is_in_applied", value=TRUE)
        
        graph <- graph.union(applied_graph, oracle_graph, byname=TRUE)
        E(graph)[is.na(E(graph)$is_in_oracle)]$is_in_oracle <- FALSE
        E(graph)[is.na(E(graph)$is_in_applied)]$is_in_applied <- FALSE
        graph <- set_edge_attr(graph, name="is_in_both", value=E(graph)$is_in_applied & E(graph)$is_in_oracle)
        
        # Set graph colors
        edge_colors <- ifelse(E(graph)$is_in_both, "forestgreen", ifelse(E(graph)$is_in_oracle, "orange", "red"))
        graph <- set_edge_attr(graph, name="edge_colors", value=edge_colors)
    }
    
    if (plot) {
        layout_custom <- layout_nicely(graph, maxiter=1000)
        graph_plot <- plot(graph, vertex.size=5, vertex.label.cex=0.6, edge.arrow.size=.3, 
             edge.color=E(graph)$edge_colors, vertex.label.color="black", vertex.frame.color="transparent", layout=layout_custom)
        print(graph_plot)
    }
...
## For each sample_ID, find all genotype matches and determine 
## which subject_ID is most represented
populate_majorities <- function(elist) {
    elist$targets %<>% group_by( E_Subject_ID ) %>%
        mutate(
            Majority_Subject_ID = ifelse(
                sum(get_mode(Subject_ID) == Subject_ID) / n() > 0.5,
                get_mode(Subject_ID), NA_character_),
            Non_Majority = Majority_Subject_ID != Subject_ID
        ) %>%
        ungroup() %>% as.data.frame()
    ## Operation above drops row names...and get a warning if I try setting
    ## row names without converting from tibble to dataframe
...
get_potential_relabels_list <- function(elist) {
    ...
    for (sample_id in inferred_mislabel_univ) {
        ...
        ## If sample_id A's genotype group has a consensus majority_subject_id
        ## The potential_relabels are sample_ids B that satisfy
        ##      1) their subject_id matches the majority_subject_id for sample A 
        ##      2) their subject_id doesn't match the majority_subject_id in its own genotype group
        if (!is.na(majority_subject_id)) {
            potential_relabels <- elist$targets %>% 
                filter(Subject_ID == majority_subject_id & (Non_Majority | is.na(Non_Majority))) %>% 
                pull(Sample_ID)
...
calc_entropy = function(labels) {
    labels %>% group_by(E_Subject_ID) %>%
        summarize(scaled_entropy = entropy(table(Subject_ID))/entropy(rep(1, n()))) %>%
        select(scaled_entropy) %>%
        sum()
}
...
objective_genotype_entropy <- function(elist, neighbors) {
    ...
    calc_delta_entropy = function(swap_from, swap_to) {
        ...
        filtered_labels <- elist$targets %>% 
            filter(E_Subject_ID %in% filtered_genotypes)
        swap_from_genotype <- filtered_labels[[swap_from, "E_Subject_ID"]]
        swap_to_genotype <- filtered_labels[[swap_to, "E_Subject_ID"]]
        swapped_labels <- filtered_labels
        swapped_labels[swap_from, "E_Subject_ID"] <- swap_to_genotype
        swapped_labels[swap_to, "E_Subject_ID"] <- swap_from_genotype
        return(calc_entropy(swapped_labels) - calc_entropy(filtered_labels))
    }
    
    neighbors %<>% mutate(
        base_entropy = elist_entropy,
        delta_entropy = mapply(calc_delta_entropy, swap_from=Sample_A, swap_to=Sample_B),
        objective = base_entropy + delta_entropy
    )
    return(neighbors)
}
...
local_search_correct <- function(elist, objective) {
    ...
    connected_components <- data.frame(
        ...
        Cluster_ID = connected_components
    )
    solved_components <- c()
    ...
    while (TRUE) {
        ...
        neighbors <- find_neighbors(combined_graph) %>%
            ...
            filter(!(Cluster_ID %in% solved_components))
        if (nrow(neighbors) == 0) { break }
        objectives <- objective_function(elist, neighbors) %>%
            group_by(Cluster_ID) %>%
            filter((delta_entropy < 0) & (delta_entropy == min(delta_entropy)))
        if (nrow(objectives) == 0) { break }
        swaps <- objectives %>%
            sample_n(1) %>%
            as.data.frame()
        solved_components <- c(solved_components, setdiff(objectives$Cluster_ID, swaps$Cluster_ID))
...
##################################################
### Start defining the mislabel handlers below ###
##################################################
...
handle_mislabels_correct_majority_cycles <- function(elist, max_cycle_size = 8) {
    while (TRUE) {
        assert_that(max_cycle_size > 1, msg=glue("max_cycle_size must be greater than 1, currently {max_cycle_size}"))
        potential_relabels_list <- get_potential_relabels_list(elist)
        
        ## Stopping condition if there are no inferred mislabels
        if (length(potential_relabels_list) == 0) {
            break
        }
        
        ## Initialize the cycle tree
        cycle_tree <- data.frame(cycle_head = names(potential_relabels_list)) %>% 
            mutate(
                cycle_path = as.list(cycle_head),
                cycle_tail = cycle_head,
                cycle_size = 0
            )
        
        ## Extend the cycle tree until a cycle has been detected
        for (curr_cycle_size in seq_len(max_cycle_size)) {
            ## Collect all leaf nodes and extend by potential_relabels
            cycle_leaf_nodes <- cycle_tree %>% filter(cycle_size == curr_cycle_size - 1) 
            cycle_leaf_nodes %<>% mutate(
                cycle_tail = lapply(cycle_tail, function(x) potential_relabels_list[[x]])
            ) %>% 
                unnest(cycle_tail) %>% 
                mutate(
                    cycle_size = curr_cycle_size,
                    cycle_path = Map(c, cycle_path, cycle_tail)
                )
            
            cycle_tree <- cycle_leaf_nodes
            
            ## Break if we've detected a cycle, or if we've reached a dead-end at each path
            if (nrow(cycle_tree) == 0 || any(cycle_tree$cycle_head == cycle_tree$cycle_tail)) { break }
        }
        
        ## Filter in complete cycles only
        cycle_tree %<>% filter(cycle_head == cycle_tail)
        
        ## Stopping condition if we've looped to max_cycle_size and haven't detected any cycles
        if (nrow(cycle_tree) == 0) {
            break
        }
        
        ## There should be no ambiguities as those are filtered out in get_potential_relabels_list
        ## This fails sometimes: refer to excel sheet. To generate plots for Monday just detect it and break out right now
        if (nrow(
            cycle_tree %>%
            group_by(cycle_head) %>% 
            filter(n() > 1)
        ...
        assert_that(
            ...
            msg = "Detected distinct cycles that return to the same head sample"
        )
        assert_that(all(cycle_tree$cycle_size > 1), msg = "Detected cycles of size < 2")
        
        relabels <- cycle_tree %>% 
            transmute(
                curr_label = cycle_path,
                new_label = lapply(cycle_path, lead)
            ) %>% 
            unnest(everything()) %>% 
...
handle_mislabels_correct_majority_swaps <- function(elist) {
    iterations <- 1
    ## Make unambiguous swaps (even ones where only one of the two mislabels in the swap get corrected)
    while (iterations < 8) {
        potential_relabels_list <- get_potential_relabels_list(elist)
        ## Filter only samples with exactly one potential relabel
        potential_relabels_list <- potential_relabels_list[lapply(potential_relabels_list, length) == 1]
        if (length(potential_relabels_list) == 0) {
            break
        }
        ## Create relabels matrix
        incomplete_relabels <- data.frame(
            relabel_from = names(potential_relabels_list)
        ) %>% mutate(
            relabel_to = sapply(relabel_from, \(x) potential_relabels_list[[x]][[1]])
        )
        
        ## First determine relabels when you know there are no cycles 
        ## (linear sequence of relabels where we can't close the cycle)
        linear_relabels <- data.frame(matrix(ncol = 2, nrow = 0))
        colnames(linear_relabels) <- colnames(incomplete_relabels)
        # Convert all columns to character type
        linear_relabels <- as.data.frame(sapply(linear_relabels, as.character))
        starts <- setdiff(incomplete_relabels$relabel_from, incomplete_relabels$relabel_to)
        for (start in starts) {
            curr_sample <- start
            while (TRUE) {
                extend_index <- which(incomplete_relabels$relabel_from == curr_sample)
                assert_that(length(extend_index) %in% c(0, 1), msg="Something is wrong in majority_swap_correct")
                ## Once you reach the end, forcefully close the cycle
                ## TODO: just remove start sample instead
                if (length(extend_index) == 0) {
                    linear_relabels <- rbind(linear_relabels, data.frame(
                        relabel_from = curr_sample,
                        relabel_to = start
                    ))
                    break
                }
                next_sample <- incomplete_relabels[[extend_index, "relabel_to"]]
                linear_relabels <- rbind(linear_relabels, data.frame(
                    relabel_from = curr_sample,
                    relabel_to = next_sample
                ))
                curr_sample <- next_sample
            }
        }
        cycle_relabels <- anti_join(incomplete_relabels, linear_relabels, by=names(incomplete_relabels))
        relabels <- rbind(cycle_relabels, linear_relabels)
        elist <- relabel_elist_samples(elist, relabels)
...
handle_mislabels_remove_majority <- function(elist) {
    ## Remove all samples where the labeled Subject_ID doesn't match the 
    ## majority Subject_ID in its genotype set
    elist <- populate_majorities(elist)
    elist <- elist[, !(elist$targets$Non_Majority | is.na(elist$targets$Non_Majority))]
    elist$targets %<>% select(-c(Majority_Subject_ID, Non_Majority))
    return(elist)
}

handle_mislabels_correct_local_search_entropy <- function(elist) {
    return(local_search_correct(elist, objective="genotype_entropy"))
}

handle_mislabels_correct_majority_cycles_with_remove_majority <- function(elist, max_cycle_size = 8) {
    elist <- handle_mislabels_correct_majority_cycles(elist, max_cycle_size)
    elist <- handle_mislabels_remove_majority(elist)
    return(elist)
}

handle_mislabels_correct_majority_cycles_swaps_with_remove_majority <- function(elist, max_cycle_size = 8) {
    elist <- handle_mislabels_correct_majority_cycles(elist, max_cycle_size)
...
handle_mislabels_correct_majority_cycles_swaps_entropy_with_remove_majority <- function(elist, max_cycle_size = 8) {
    elist <- handle_mislabels_correct_majority_cycles(elist, max_cycle_size)
    elist <- handle_mislabels_correct_majority_swaps(elist)
    elist <- handle_mislabels_correct_local_search_entropy(elist)
    elist <- handle_mislabels_remove_majority(elist)
    return(elist)
}
...
sim_mislabeled_data <- function (
    ...
    handle_swaps_function <- swap_handlers[[swap_model]] #eval(parse(text=swap_model))
...
grid_list <- list(
    ...
    tibble(n_mislabels = seq(from = 60, to = 60, by = 10)),
...
sim_table <- grid_list %>%
    Reduce(f = cross_join) %>%
    distinct %>%
    filter(!(handle_mislabels == "remove" & n_mislabels >= 2 * n_subjects_per_group * n_samples_per_subject)) %>%
    arrange(seed)
...
sim_table %<>% filter(handle_mislabels != "correct_local_search_entropy") %>%
    filter(!(handle_mislabels == "majority_cycles_swap_entropy_correct_and_remove" & n_mislabels > 130))
...
if (file_exists("sim_results.qs")) {
    ...
    sim_result_table <- sim_table %>%
        mutate(
            results_list = list(try(stop("Simulation not run yet"), silent = TRUE)),
...
## To comment out once weird bug is fixed
plan(sequential)
plan(callr)
time1 <- Sys.time()
...
sim_result_perf_table <- sim_result_long_tt_table %>%
    ...
    mutate(
      Pct_Swapped = 100 * n_mislabels / (2 * n_samples_per_subject * n_subjects_per_group),
...
n_samples <- max(2*sim_result_perf_summary_table$n_samples_per_subject*sim_result_perf_summary_table$n_subjects_per_group, na.rm=TRUE)
mislabel_handlers_to_plot <- c("remove_majority", "correct_majority_cycles", "correct_majority_swaps", 
                               "correct_majority_cycles_swaps_with_remove_majority", "correct_majority_cycles_swaps_entropy_with_remove_majority")
