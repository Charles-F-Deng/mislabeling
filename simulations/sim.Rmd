...
reorder_elist_samples <- function(elist, sample_ids) {
    ...
    reordered_elist$targets %<>% mutate(
        ...
        E_Subject_ID = lapply(sample_ids, \(x) str_split_1(x, "_")[1]),
...
sim_mislabeled_data <- function (
    ...
    ## TODO: add swapping with replacement 
    ## Generate swaps
    if (swap_systematic) {
        swaps <- cbind(
    ...
    ## Store targets after swaps so we can access original Sample_ID/E_Sample_ID
    ## pairs and evaluate the results of the label correction algorithm
...
run_sim <- function(..., handle_mislabels = handle_mislabels_options, save_eta_matrices = FALSE) {
    ...
    label_outcomes <- elist$orig$targets_orig %>%
