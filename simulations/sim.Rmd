...
plot_graph <- function(graph) {
    ...
    my_plot <- plot(graph, vertex.size=4, vertex.label.cex=2, edge.arrow.size=.8, edge.width=5, 
...
correct_comprehensive_search <- function(sample_genotype_data, max_subjects=9) {
    relabels <- data.frame(relabel_from=character(0), relabel_to=character(0))
...
correct_main <- function(sample_genotype_data) {
    validate_sample_genotype_data(sample_genotype_data)
    ...
    for (i in seq_along(sorted_components)) {
        ...
        ordered_components[components == sorted_components[i]] <- glue("Component_", formatted_component)
    }
    ...
    while (TRUE) {
        sample_genotype_data <- correct_comprehensive_search(sample_genotype_data)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
        sample_genotype_data <- correct_majority_cycles(sample_genotype_data)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
        sample_genotype_data <- correct_majority_cycles(sample_genotype_data)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
        sample_genotype_data <- correct_local_search_entropy(sample_genotype_data, n_iter=1)
        if (objective_hamming_distance(sample_genotype_data) == 0) break
    }
...
correct_main(sample_genotype_data)
