...
## Helper function for the iterative_majority algorithm
...
mislabel_handlers <- list(
    ...
    majority_swap_correct = function(elist) {
        ...
        while (TRUE) {
            ...
            ## Create swap matrix
            swaps <- data.frame(
                swap_a = names(potential_relabels_list)
            ) %>% mutate(
                swap_b = sapply(swap_a, \(x) potential_relabels_list[[x]][[1]])   
            ) %>% transmute(
                swap_from = pmin(swap_a, swap_b),
                swap_to = pmax(swap_a, swap_b)
            ) %>% distinct()
            elist <- swap_elist_samples(elist, swaps)
        }
        return(elist)
...
mislabel_handlers_to_plot <- c("majority_remove", "majority_cycles_correct", "majority_cycles_correct_and_remove")
