...
# Right now it just reorders exp data
...
detect_cycles <- function(elist) {
    mislabeled_elist <- elist$targets %>% filter(Mislabeled) %>% select(Sample_ID, E_Sample_ID)
    ...
    majority_correct = function(elist) {
        ...
        sample_mislabel_table <- elist$targets %>%
            ...
            mutate(
                ...
                Relabeled_Samples = ifelse(Correctable, Sample_ID, E_Sample_ID)
            )
...
sim_mislabeled_data <- function (
    ...
    # reordered_elist <- handle_swaps_function(elist, n_mislabels)
...
grid_list <- list(
    ...
    tibble(n_mislabels = seq(from = 0, to = 300, by = 30)),
