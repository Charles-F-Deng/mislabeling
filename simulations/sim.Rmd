...
swap_elist_samples <- function(elist, swaps) {
    ...
    for (row in seq_len(nrow(swaps))) {
        sample1 <- swaps[row, 1]
        sample2 <- swaps[row, 2]
        index1 <- match(sample1, elist$targets$E_Sample_ID)
        index2 <- match(sample2, elist$targets$E_Sample_ID)
...
sim_mislabeled_data <- function (
    ...
    sex_sd = 0,
    age_sd = 0,
    swap_model = c("systematic", "random"),
    seed
) {
    if (!missing(seed)) {
        sim_params <- mget(c(
    ...
    assert_that(
        ...
        sex_sd >= 0,
        age_sd >= 0,
        swap_model %in% c("systematic", "random")
    )
    if (n_sv < 1 && sv_sd > 0) {
    ...
    n_swaps_temp <- floor(n_mislabels / 2)
    
    ## Generate swaps
    if (swap_model == "systematic") {
        swaps <- cbind(
            sample(ctrl_samples, n_swaps_temp),
            sample(case_samples, n_swaps_temp)
        )
    } else if (swap_model == "random") {
        swaps <- matrix(sample(sample_meta_table$Sample_ID, n_swaps_temp * 2), ncol = 2)
    }
    ## Apply swaps to expression matrix thus generating mislabeled samples
    reordered_elist <- swap_elist_samples(elist, swaps)
    ...
    reordered_elist$origs$targets_orig <- reordered_elist$targets
...
grid_list <- list(
    ...
    tibble(swap_model = c("systematic", "random")),
    ...
    tibble(seed = seq(from = 1986, length.out = 1))
)
