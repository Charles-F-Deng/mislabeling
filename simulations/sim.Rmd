...
mislabel_handlers <- list(
    ...
    iterative_majority = function(elist) {
        ...
        while (TRUE) {
            ...
            cycle_tree <- data.frame(head = names(potential_relabels_list)) %>% 
                mutate(
                    path = head,
                    tail = head,
                    cycle_size = 1
            ...
            for (i in 1:nrow(cycle_tree)) {
                ...
                ## Correct elist
                elist <- swap_elist_samples(elist, swaps)
                corrected_labels <- c(corrected_labels, path)
            }
        }
    }
)
...
grid_list <- list(
    ...
    tibble(n_mislabels = seq(from = 0, to = 30, by = 30)),
