...
swap_elist_samples <- function(elist, swaps) {
    ...
    for (row in 1:nrow(swaps)) {
        sample1 <- swaps[row, 1]
        sample2 <- swaps[row, 2]
        index1 <- match(sample1, sample_ids)
        index2 <- match(sample2, sample_ids)
...
local_search_objectives <- list(
    genotype_entropy = function(elist, neighbors) {
        ...
        elist_labels <- elist$targets[, c("Sample_ID", "Subject_ID", "E_Subject_ID")]
        elist_entropy <- calc_entropy(elist_labels)
        
        calc_delta_entropy = function(swap_from, swap_to) {
            filtered_genotypes <- elist_labels %>%
                filter(Sample_ID %in% c(swap_from, swap_to)) %>%
                pull(E_Subject_ID)
            filtered_labels <- elist_labels %>% filter(E_Subject_ID %in% filtered_genotypes)
...
objective_function_options <- names(local_search_objectives)
...
mislabel_handlers <- list(
    ...
    local_search_correct = function(elist, objective=objective_function_options) {
        
        objective <- as.character(objective)
        objective <- match.arg(objective)
        if (objective %in% objective_function_options) {
            objective_function <- local_search_objectives[[objective]]
        } else {
    ...
    majority_swap_correct = function(elist) {
        iterations <- 1
    ...
    majority_cycles_correct = function(elist, max_cycle_size = 8) {
        while (TRUE) {
    ...
    majority_cycles_correct_and_remove = function(elist, max_cycle_size = 8) {
        elist <- mislabel_handlers$majority_cycles_correct(elist, max_cycle_size)
        elist <- mislabel_handlers$majority_remove(elist)
        return(elist)
    },
    majority_cycles_swap_correct_and_remove = function(elist, max_cycle_size = 8) {
        elist <- mislabel_handlers$majority_cycles_correct(elist, max_cycle_size)
        elist <- mislabel_handlers$majority_swap_correct(elist)
        elist <- mislabel_handlers$majority_remove(elist)
        return(elist)
    },
    majority_cycles_swap_entropy_correct_and_remove = function(elist, max_cycle_size = 8) {
        elist <- mislabel_handlers$majority_cycles_correct(elist, max_cycle_size)
        elist <- mislabel_handlers$majority_swap_correct(elist)
        elist <- mislabel_handlers$local_search_correct(elist, "genotype_entropy")
        elist <- mislabel_handlers$majority_remove(elist)
        return(elist)
    }
)
...
generate_graph = function(elist, graph_type=names(graph_type_mapping)) {
    ...
    if (graph_type == "combined") {
        ...
        return(union(genotype_graph, labels_graph))
    }
...
sim_mislabeled_data <- function (
    ...
    if (swap_model == "random") {
        ...
        ## Apply swaps to expression matrix thus generating mislabeled samples
        elist <- swap_elist_samples(elist, swaps)
    } else if (swap_model == "random_with_replacement") {
        n_correct_labels <- nrow(elist$targets %>% filter(!Mislabeled))
        assert_that(n_mislabels <= n_correct_labels,
               msg = glue("Can't generate {n_mislabels} new mislabels when there are only {n_correct_samples} correctly labeled samples"))
        n_curr_mislabels <- nrow(elist$targets %>% filter(Mislabeled))
...
grid_list <- list(
    ...
    tibble(n_mislabels = seq(from = 0, to = 300, by = 10)),
...
sim_table %<>% filter(handle_mislabels != "local_search_correct") %>% 
    filter(!(handle_mislabels == "majority_cycles_swap_entropy_correct_and_remove" & n_mislabels > 90))
```
...
mislabel_handlers_to_plot <- c("majority_remove", "majority_cycles_correct_and_remove", "majority_swap_correct", 
                               "majority_cycles_swap_correct_and_remove", "majority_cycles_swap_entropy_correct_and_remove")
...
perf_yx_intercept <- if_else(perf_names %in% mislabels_perf_names, 0, n_samples) %>%
    set_names(perf_names)
assert_that(
    all(mislabel_handlers_to_plot %in% sim_result_perf_summary_table$handle_mislabels),
    msg = "Specified mislabel handle methods that were not part of simulation grid")
...
elist1 <- mislabel_handlers$majority_cycles_swap_correct_and_remove(elist)
elist2 <- mislabel_handlers$majority_cycles_swap_entropy_correct_and_remove(elist)

sim_params <- elist_params
sim_params$handle_mislabels <- "all_remove"
