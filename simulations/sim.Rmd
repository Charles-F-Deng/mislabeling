...
generate_graph = function(elist, graph_type=names(graph_type_mapping), oracle=FALSE, plot=FALSE) {
    ...
    if (graph_type == "combined") {
        ...
        if (oracle) {
            ...
            V(graph)$vertex_colors <- ifelse(V(graph)$Mislabeled, "firebrick2", "forestgreen")
        }
        if (plot) { 
            plot_graph(graph) 
        }
...
get_majority_majority_relabels_graph <- function(elist) {
    votes <- genotype_vote(elist$targets)
...
##################################################################
################    SUBJECT_ID/GENOTYPE SEARCH    ################
##################################################################

subject_genotype_search <- function(connected_component, lock_majority_majority=TRUE) {
...
handle_mislabels_correct_main_with_remove_majority <- function(elist, n_iter=1, max_subjects=9, n_set=10) {
    curr_set <- 0
    while (curr_set < n_set) {
        ...
        elist <- handle_mislabels_correct_local_search_entropy(elist, n_iter=n_iter)
        curr_set <- curr_set + 1
    }
